...
export async function GET(req: Request) {
  ...
  const [{ data: turco, error: eTurco }, { data: peral, error: ePeral }] = await Promise.all([
    sb.from("registros_turco").select("*").order("fecha_hora", { ascending: false }).limit(limit),
    sb.from("registros_peral").select("*").order("fecha_hora", { ascending: false }).limit(limit),
  ]);

  if (eTurco) console.error("[centro-data] turco:", eTurco.message);
  if (ePeral) console.error("[centro-data] peral:", ePeral.message);

  return NextResponse.json({ turco: turco ?? [], peral: peral ?? [] });
}
